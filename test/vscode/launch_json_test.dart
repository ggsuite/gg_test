...
void main() {
  ...
  group('.vscode/launch.json', () {
    test('pathes in launch.json', () {
      ...
      // Ensure there is a configuration for executing bin/gg_test.dart
      final configurations =
          parsedLaunchJson['configurations'] as List<dynamic>;

      final ggTest = configurations.firstWhere(
        (dynamic configuration) =>
            configuration['name'].toString() == 'gg_test.dart',
      );

      expect(
        ggTest,
        isNotNull,
        reason: 'Item for executing bin/gg_test.dart not found',
      );

      expect(ggTest['name'], 'gg_test.dart');
      expect(ggTest['type'], 'dart');
      expect(ggTest['request'], 'launch');
