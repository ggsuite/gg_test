...
2. Runs the test process through an injectable `GgProcessWrapper` (from `gg_process`) so tests can mock I/O. `--coverage coverage` is passed for Dart; `--coverage` for Flutter. For Dart, coverage collection is additionally scoped to the package's own code via `--coverage-package '^<name>$'` when the resolved `test_core` is >= 0.6.15 (read from `pubspec.lock`, walking up parent directories for pub workspaces; see `coveragePackageArgs`). Otherwise it falls back to unscoped collection, which is much slower because the VM reports coverage for every dependency.
